...
const fetchGDACSDisasters = async () => {
    ...
    // Filter to only include disasters from the current year
    const currentYear = moment().year();
    const currentYearDisasters = disasters.filter(disaster =>
        moment(disaster.pubDate).year() === currentYear
    );

    // Sort by newest first
    currentYearDisasters.sort((a, b) => new Date(b.pubDate) - new Date(a.pubDate));
    return currentYearDisasters

};

module.exports = fetchGDACSDisasters;
